...
async function getAllFilePathsInRepo(owner: string, repo: string, ref: string = 'HEAD'): Promise<string[]> {
  ...
  return data.tree.filter((file:any) => file.type === 'blob').map((file:any) => file.path || '');
}
...
export async function getAllReferences(
  ...
  for (const query of list_of_queries) {
    ...
    for (const file_path of files_to_search) {
      ...
      const file_content = await getFileContent(owner, repo, file_path, branch_name);
